...
month_days = [
    None, 31, 29 if leap_year(today_mock.year) else 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31
]
...
class TestDeltaTime(ParserTestCase):
    def assertParses(self, data):
        for test in data:
            ...
            if isinstance(expected_date, datetime):
                expected_dict = {'datetime': expected_date.isoformat(' ')}
                ...
                expected_dict = {'date': expected_date.isoformat()}
            elif isinstance(expected_date, time):
                expected_dict = {'time': expected_date.isoformat()}
            else:
    ...
    def assertParsingError(self, data):
        for text in data:
            expected_dict = {'datetime_parsing_error': True}
            self.assertParsed(kronosparser.delta_time, text, expected_dict)

    def test_no_date(self):
        invalid = {'datetime_parsing_error': True}
        self.assertParsed(kronosparser.delta_time,
                          'there is no date in 2223 Raspberry Ln Mountain View CA 94045', [invalid])
        self.assertParsed(
            kronosparser.delta_time,
            'I drove from 1729A 4th Avenue North in Nashville to Murfreesboro, Tennessee.', [])
        self.assertParsed(kronosparser.delta_time, 'know', [])
        self.assertParsed(kronosparser.delta_time, 'nowhere', [])
        self.assertParsed(kronosparser.delta_time, 'good morning', [invalid])
        self.assertParsed(kronosparser.delta_time, 'good afternoon', [invalid])
        self.assertParsed(kronosparser.delta_time, 'good evening', [invalid])
        self.assertParsed(kronosparser.delta_time, 'good night', [invalid])
        self.assertParsed(kronosparser.delta_time, 'Macbook pro $1999', [invalid])

    def test_bad_date(self):
        data = ('2015-13-12', '2015/20/11')
    ...
    def test_yesterday(self, utc_today_mock, utc_now_mock):
        ...
        data = (('had lunch with the team yesterday', yesterday, {
            'days_delta': 1,
            'tz_threshold': 24 - hour,
            'utc': False
        } if hour >= 12 else {
    ...
    def test_weekday(self, utc_today_mock, utc_now_mock):
        ...
        last_tuesday_noon = {
            'datetime': datetime.combine(get_last_tuesday(), t_noon).isoformat(' ')
        }
        next_tuesday_noon = {
            'datetime': datetime.combine(get_next_tuesday(), t_noon).isoformat(' ')
        }
        if today.weekday() == 1 and hour >= 12:
    ...
    def test_specific_date(self, utc_today_mock):
        data = (('had lunch Tuesday March 15, 2016', date(2016, 3, 15), {
            'utc': False
        }), ('had lunch November 12, 2015', date(2015, 11, 12), {
            'utc': False
        }), ('had lunch Nov 23 2015', date(2015, 11, 23), {
            'utc': False
        }), ('had lunch 11-24-2015', date(2015, 11, 24), {
            'utc': False
        }), ('had lunch sept. 25 2015', date(2015, 9, 25), {
            'utc': False
        }), ('had lunch in restaurant junipero 16 the sept. 26 2015', date(2015, 9, 26), {
            'utc': False
        }), ('had lunch sept. 27th 2011', date(2011, 9, 27), {
            'utc': False
        }), ('had lunch sept. the 28th 2011', date(2011, 9, 28), {
            'utc': False
        }), ('had lunch 2015-11-29', date(2015, 11, 29), {
            'utc': False
        }), ('had lunch 2015-11-29', date(2015, 11, 29), {
            'utc': False
        }), ('had lunch 1/12/15', date(2015, 1, 12), {
            'utc': False
        }), ('had lunch 11/12/15', date(2015, 11, 12), {
            'utc': False
        }), ('had lunch 1/2/15', date(2015, 1, 2), {
            'utc': False
        }), ('had lunch 01/2/15', date(2015, 1, 2), {
            'utc': False
        }), ('had lunch 1/02/15', date(2015, 1, 2), {
            'utc': False
        }), ('Burger king 8.86 4/07/12', date(2012, 4, 7), {
            'utc': False
        }), ('next week Tuesday March 15, 2016', date(2016, 3, 15), {
            'utc': False
        }))
        self.assertParses(data)
    ...
    def test_date_no_year_no_month(self, utc_today_mock, utc_now_mock):
        ...
        if tomorrow.day == 1 and hour >= 12:
            ...
            flags_18 = dict(zip(FLAG_KEYS, (False, 28, 24 - hour))) if today.day == 28 else {
                'utc': False
            }
            ...
            flags_18 = dict(zip(FLAG_KEYS, (False, 28, -hour - 1))) if yesterday.day == 28 else {
                'utc': False
            }
        else:
            flags_18 = {'utc': False}
        ...
        data = (('the 8th', date(today.year, today.month, 8), flags), )
        self.assertParses(data)

        wednesday_text = 'Wednesday 18th'
        if candidate_month_18.weekday() is 2:
            data = ((wednesday_text, candidate_month_18, flags_18), )
            self.assertParses(data)
        else:
            expected_dict = {'datetime_parsing_error': True}
    ...
    @unittest.skipIf(utc_today_mock().day in [1, month_days[utc_today_mock().month]],
    ...
    def test_imprecise_dates(self, utc_today_mock, utc_now_mock):
        ...
        data = (('1st of this month', today - timedelta(days=today.day - 1), {
            'utc': False
        }), ('a year ago', year_ago, flags), ('a month ago', month_ago, flags if any(
            day.day < month_days[month_ago.month] for day in [today, alt_day]) else {
                'utc': False
            }), ('a week ago', week_ago, flags), ('3 weeks ago', three_weeks_ago, flags),
                ('three weeks ago', three_weeks_ago,
                 flags), ('40 minutes ago', utc_now_mock() - timedelta(minutes=40), {
                     'utc': True
                 }), ('1 hour ago', utc_now_mock() - timedelta(hours=1), {
                     'utc': True
                 }), ('1 minute ago', utc_now_mock() - timedelta(minutes=1), {
                     'utc': True
                 }), ('in a year', in_a_year, flags),
                ('in a month', in_a_month, flags if any(day.day < month_days[in_a_month.month]
                                                        for day in [today, alt_day]) else {
                                                            'utc': False
    ...
    def test_beginning_and_end_of(self, utc_today_mock, utc_now_mock):
        ...
        mid_next_month = date(today.year + this_month // 12, this_month % 12 + 1,
                              mid_days[this_month % 12 + 1])
        end_next_month = date(today.year + this_month // 12, this_month % 12 + 1,
                              last_day_next_month)
        mid_last_year = date(today.year - 1, 7, 1 if leap_year(today.year - 1) else 2)
        data = (('end of jan', date(today.year, 1, 31), {
            'utc': False
        }), ('end of week', get_week_start() + timedelta(days=6), {
            'utc': False
        }), ('end of 2014', date(2014, 12, 31), {
            'utc': False
        }), ('end of last year', date(today.year - 1, 12, 31), {
            'utc': False
        }), ('end of yesterday', today - timedelta(days=1), {
            'utc': False
        }), ('end of today', today, {
            'utc': False
        }), ('end of this year', date(today.year, 12, 31), {
            'utc': False
        }), ('end of next year', date(today.year + 1, 12, 31), {
            'utc': False
        }), ('end of this month', date(today.year, this_month, last_day), {
            'utc': False
        }), ('end of next month', end_next_month, {
            'utc': False
        }), ('end of Q1', date(today.year, 3, 31), {
            'utc': False
        }), ('late jan', date(today.year, 1, 31), {
            'utc': False
        }), ('late 2014', date(2014, 12, 31), {
            'utc': False
        }), ('late last year', date(today.year - 1, 12, 31), {
            'utc': False
        }), ('late yesterday', today - timedelta(days=1), {
            'utc': False
        }), ('late today', today, {
            'utc': False
        }), ('late this year', date(today.year, 12, 31), {
            'utc': False
        }), ('late next year', date(today.year + 1, 12, 31), {
            'utc': False
        }), ('late this month', date(today.year, this_month, last_day), {
            'utc': False
        }), ('late next month', end_next_month, {
            'utc': False
        }), ('late second quarter', date(today.year, 6, 30), {
            'utc': False
        }), ('beginning of next month', date(today.year + this_month // 12, this_month % 12 + 1, 1),
             {
                 'utc': False
             }), ('start of week', get_week_start(), {
                 'utc': False
             }), ('beginning of january', date(today.year, 1, 1), {
                 'utc': False
             }), ('beginning of 96', date(1996, 1, 1), {
                 'utc': False
             }), ('beginning of last year', date(today.year - 1, 1, 1), {
                 'utc': False
             }), ('beginning of yesterday', today - timedelta(days=1), {
                 'utc': False
             }), ('beginning of today', today, {
                 'utc': False
             }), ('beginning of Q3', date(today.year, 7, 1), {
                 'utc': False
             }), ('early next month', date(today.year + this_month // 12, this_month % 12 + 1, 1), {
                 'utc': False
             }), ('early january', date(today.year, 1, 1), {
                 'utc': False
             }), ('early 96', date(1996, 1, 1), {
                 'utc': False
             }), ('early last year', date(today.year - 1, 1, 1), {
                 'utc': False
             }), ('early yesterday', today - timedelta(days=1), {
                 'utc': False
             }), ('early today', today, {
                 'utc': False
             }), ('early Q4', date(today.year, 10, 1), {
                 'utc': False
             }), ('mid next month', mid_next_month, {
                 'utc': False
             }), ('mid week', get_week_start() + timedelta(days=3), {
                 'utc': False
             }), ('mid-next week', get_week_start() + timedelta(days=10), {
                 'utc': False
             }), ('mid january', date(today.year, 1, 16), {
                 'utc': False
             }), ('mid 96', date(1996, 7, 1), {
                 'utc': False
             }), ('mid last year', mid_last_year, {
                 'utc': False
             }), ('mid yesterday', today - timedelta(days=1), {
                 'utc': False
             }), ('mid today', today, {
                 'utc': False
             }), ('mid Q1', mid_q1, {
                 'utc': False
             }), ('middle of next month', mid_next_month, {
                 'utc': False
             }), ('middle of week', get_week_start() + timedelta(days=3), {
                 'utc': False
             }), ('middle of january', date(today.year, 1, 16), {
                 'utc': False
             }), ('middle of 96', date(1996, 7, 1), {
                 'utc': False
             }), ('middle of last year', mid_last_year, {
                 'utc': False
             }), ('middle of yesterday', today - timedelta(days=1), {
                 'utc': False
             }), ('middle of today', today, {
                 'utc': False
             }), ('middle of Q1', mid_q1, {
                 'utc': False
             }))
    ...
    def test_date_with_time(self, utc_today_mock, utc_now_mock):
        ...
        data = (
            ('yesterday 0:15:20',
             datetime.combine(utc_today_mock(), time(0, 15, 20)) - timedelta(days=1), day_flags),
            ('yesterday around 8am',
             datetime.combine(utc_today_mock(), time(8)) - timedelta(days=1), day_flags),
            ('today 12:45', datetime.combine(utc_today_mock(), time(12, 45)), day_flags),
            ('tomorrow 00:15:21',
             datetime.combine(utc_today_mock(), time(0, 15, 21)) + timedelta(days=1), day_flags),
            ('noon tomorrow', datetime.combine(utc_today_mock(), t_noon) + timedelta(days=1),
             day_flags),
            ("4 o'clock tomorrow", datetime.combine(utc_today_mock(), time(16)) + timedelta(days=1),
             day_flags),
            ("4 o'clock CT tomorrow",
             datetime.combine(utc_today_mock(), time(16)) + timedelta(days=1), day_flags),
            ('tomorrow noon', datetime.combine(utc_today_mock(), t_noon) + timedelta(days=1),
             day_flags),
            ('tomorrow at noon', datetime.combine(utc_today_mock(), t_noon) + timedelta(days=1),
             day_flags),
            ('05/20 15:20', datetime(utc_now_mock().year, 5, 20, hour=15,
                                     minute=20), flags_after_29),
            ('05/21 at 15:25', datetime(utc_now_mock().year, 5, 21, hour=15,
                                        minute=25), flags_after_29),
            ('05/21 @ 15:26', datetime(utc_now_mock().year, 5, 21, hour=15,
                                       minute=26), flags_after_29),
            ('Apollo 11 landed on July 20, 1969, at 13:18', datetime(1969,
                                                                     7,
                                                                     20,
                                                                     hour=13,
                                                                     minute=18), {
            ...
            ('Dec 31 1979 at 17:00', datetime(1979, 12, 31, hour=17), {
                'utc': False
            }),
            ('in two days at 2pm', datetime.combine(utc_today_mock(), time(14)) + timedelta(days=2),
             day_flags),
        )
        self.assertParses(data)
    ...
    def test_general_relative_dates(self, utc_today_mock, utc_now_mock):
        ...
        flags = {'utc': False}
        flags_before_29, flags_after_29 = no_year_flags()
        data = (('2 days before Nov 23 2015', date(2015, 11, 23) - timedelta(days=2),
                 flags), ('in 23 years from Jan 1, 2016', date(2039, 1, 1), flags),
                ('2 weeks before Dec 24', date(current_year, 12, 24) - timedelta(weeks=2),
                 flags_after_29), ('2 months after Dec 25', date(current_year + 1, 2,
                                                                 25), flags_after_29),
                ('3 months after Dec 25', date(current_year + 1, 3, 25),
                 flags_after_29), ('a year after Dec 26', date(current_year + 1, 12,
                                                               26), flags_after_29))
        self.assertParses(data)
    ...
    def test_part_of_day(self, utc_today_mock, utc_now_mock):
        ...
        data = (
            ...
            ('end of day', datetime.combine(utc_today_mock(), t_eod), {
                'utc': False
            }),  # TODO fix flags for intervals
    ...
    def test_date_with_part_of_day(self, utc_today_mock, utc_now_mock):
        ...
        data = (
            ...
            ('05/20 in the afternoon',
             datetime.combine(date(utc_now_mock().year, 5, 20), t_afternoon), flags_after_29),
            ('05/21/2015 evening', datetime.combine(date(2015, 5, 21), t_evening), {
                'utc': False
            }),
            ('05/22/12 in the evening', datetime.combine(date(2012, 5, 22), t_evening), {
                'utc': False
            }),
            ('6/27 98 at night', datetime.combine(date(1998, 6, 27), t_night), {
                'utc': False
            }),
            ('Dec 12, 2012 night', datetime.combine(date(2012, 12, 12), t_night), {
                'utc': False
            }),
        )
        self.assertParses(data)
    ...
    def test_interval(self, utc_today_mock, utc_now_mock):
        ...
        if month == 1:
            ...
            last_month = TimeInterval(date(year, month - 1, 1),
                                      date(year, month - 1, month_days[month - 1]))
        if month == 12:
            next_month = TimeInterval(date(year + 1, 1, 1), date(year + 1, 1, 31))
        else:
            next_month = TimeInterval(date(year, month + 1, 1),
                                      date(year, month + 1, month_days[month + 1]))
        ...
        data = (
            ...
            ('last week', TimeInterval(last_week_s, last_week_s + timedelta(days=6)), {
                'utc': False
            }),
            ('this week', TimeInterval(this_week_s, this_week_s + timedelta(days=6)), {
                'utc': False
            }),
            ('next week', TimeInterval(next_week_s, next_week_s + timedelta(days=6)), {
                'utc': False
            }),
            ...
            ('last year', TimeInterval(date(year - 1, 1, 1), date(year - 1, 12, 31)), {
                'utc': False
            }),
            ('this year', TimeInterval(date(year, 1, 1), date(year, 12, 31)), {
                'utc': False
            }),
            ('next year', TimeInterval(date(year + 1, 1, 1), date(year + 1, 12, 31)), {
                'utc': False
            }),
            ('Q1', TimeInterval(date(year, 1, 1), date(year, 3, 31)), {
                'utc': False
            }),
            ('November', TimeInterval(date(year, 11, 1), date(year, 11, 30)), {
                'utc': False
            }),
            ('February 2012', TimeInterval(date(2012, 2, 1), date(2012, 2, 29)), {
                'utc': False
            }),
            ('July, 1992', TimeInterval(date(1992, 7, 1), date(1992, 7, 31)), {
                'utc': False
            }),
            ('Q1 2015', TimeInterval(date(2015, 1, 1), date(2015, 3, 31)), {
                'utc': False
            }),
            ('show me 2017 opportunities', TimeInterval(date(2017, 1, 1), date(2017, 12, 31)), {
                'utc': False
            }),
            ('this quarter', get_this_quarter(), {
                'utc': False
            }),
            ('next quarter', get_next_quarter(), {
                'utc': False
            }),
            ('third quarter 2017', TimeInterval(date(2017, 7, 1), date(2017, 9, 30)), {
                'utc': False
            }),
            ('later half of january 2017', TimeInterval(date(2017, 1, 16), date(2017, 1, 31)), {
                'utc': False
            }),
        )
        self.assertParses(data)
    ...
    def test_date_range(self, patched_utc_today, patched_utc_now):
        ...
        data = (
            ('before 2015-04-03 after 2012-12-25', TimeInterval(date(2012, 12, 26),
                                                                date(2015, 4, 2)), {
                                                                    'utc': False
                                                                }),
            ('wed jan 11 2017 5:00 pm - 7:30 pm',
             TimeInterval(datetime(2017, 1, 11, hour=17), datetime(2017, 1, 11, hour=19,
                                                                   minute=30)), {
                                                                       'utc': False
                                                                   }),
            ('from 2:30pm to 3pm on next tuesday',
             TimeInterval(datetime.combine(next_tue, time(hour=14, minute=30)),
                          datetime.combine(next_tue, time(hour=15))), next_tuesday_flags),
            ('from 2016-01-01 to 2016-12-31', TimeInterval(date(2016, 1, 1), date(2016, 12, 31)), {
                'utc': False
            }),
            ('between 2017-01-01 and 2017-12-31', TimeInterval(date(2017, 1, 1),
                                                               date(2017, 12, 31)), {
                                                                   'utc': False
                                                               }),
        )
    ...
    def test_asap(self, utc_today_mock, utc_now_mock):
        next_day_8 = utc_today_mock() if utc_now_mock().hour < 8 else utc_today_mock() + timedelta(
            days=1)
        flags = {
            'utc':
            False,
            'days_delta':
            -1 if 8 <= utc_now_mock().hour < 20 else 1,
            'tz_threshold':
            8 - utc_now_mock().hour if utc_now_mock().hour < 8 else 7 -
            utc_now_mock().hour if utc_now_mock().hour < 20 else 32 - utc_now_mock().hour
        }
        data = (('asap', datetime.combine(next_day_8, time(8)), flags), )
        self.assertParses(data)
